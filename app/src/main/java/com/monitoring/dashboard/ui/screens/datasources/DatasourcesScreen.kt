package com.monitoring.dashboard.ui.screens.datasources
...
data class DatasourcesUiState(
    ...
    val datasources: List<DatasourceDto> = emptyList(),
...
class DatasourcesViewModel @Inject constructor(
    ...
    private val _uiState = MutableStateFlow(DatasourcesUiState())
    ...
    fun load() {
        viewModelScope.launch {
            ...
            when (val result = grafanaRepository.getDatasources()) {
                is NetworkResult.Success -> _uiState.update {
                    it.copy(isLoading = false, datasources = result.data)
                }
...
fun DatasourcesScreen(
    ...
    Scaffold(
        topBar = {
            TopAppBar(
                title = { Text(stringResource(R.string.screen_datasources_title)) },
                navigationIcon = {
        ...
        when {
            ...
            uiState.datasources.isEmpty() -> EmptyState(
                message = stringResource(R.string.datasources_empty),
                modifier = Modifier.padding(padding),
            )
            else -> LazyColumn(
                modifier = Modifier
                ...
                items(uiState.datasources, key = { it.id }) { ds ->
                    MonitoringCard(
                        title = ds.name,
                        subtitle = listOfNotNull(ds.type, ds.url).joinToString(" · "),
